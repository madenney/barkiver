...
const configDir = path.resolve(app.getPath('appData'),'barkiver')
const configPath = path.resolve(configDir, 'barkiver.json')

ipcMain.on('getMatches', async (event) => {
  const matches = require("../../data.json")
  event.reply('matches', matches)
})
